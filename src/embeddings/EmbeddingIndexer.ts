...
export class EmbeddingIndexer {
  ...
  async getStats(): Promise<EmbeddingStats> {
    ...
    const embeddedResult = await db.get(
    ...
    const failedResult = await db.get(
      'SELECT COUNT(*) as count FROM chunks WHERE embedding = ?',
   ...
   */
  async getPendingChunks(batchSize: number = 100): Promise<Array<{ id: number; content: string }>> {
    const db = await getDatabase();
   ...
   */
  async generateEmbeddings(batchSize: number = 10): Promise<EmbeddingProgress> {
    const db = await getDatabase();
    const progress: EmbeddingProgress = { total: 0, processed: 0, success: 0, failed: 0 };
      ...
      for (const chunk of chunks) {
        try {
          ...
          if (result.embedding.length === 0) {
            ...
            // Store the embedding
            const embeddingBuffer = EmbeddingService.serializeEmbedding(result.embedding);
   ...
   */
  private async storeEmbedding(chunkId: number, embeddingBuffer: Buffer): Promise<void> {
    const db = await getDatabase();
    await db.run(
      'UPDATE chunks SET embedding = ? WHERE id = ?',
      [embeddingBuffer, chunkId]
    );
  }
   ...
   */
  private async markChunkAsFailed(chunkId: number): Promise<void> {
    const db = await getDatabase();
   ...
   */
  async regenerateAllEmbeddings(): Promise<EmbeddingProgress> {
    const db = await getDatabase();
  ...
  async getEmbedding(chunkId: number): Promise<number[] | null> {
    ...
    return EmbeddingService.deserializeEmbedding(buffer);
  }
}
